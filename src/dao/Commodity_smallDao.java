...
import dao.impl.Commodity_smallDaoImpl;
...
public class Commodity_smallDao extends BaseDao implements Commodity_smallDaoImpl {
    @Override
    public List<Commodity_small> commBySmall(int id) {
        ...
        try {
            while (rs.next()){
                Commodity_small comm = new Commodity_small(rs.getInt("commodity_small_id"),rs.getString("commodity_small_name"),new CommodityDao().commById(rs.getInt("commodity_id")));
                list.add(comm);
            }
