...
public class InitServlet extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if("findShow".equals(op)){
            ...
            //            显示奶粉品牌
            int ret = new BrandService().listBrand();
//            加载类别
            List<Commodity> comm = new CommodityDao().getListByAll();
            List<List<Commodity_small>> small = new ArrayList<>();
            for(int i = 0 ; i < comm.size() ; i ++ ){
                List<Commodity_small> smalls = new Commodity_smallDao().commBySmall(comm.get(i).getCommodity_id());
                small.add(smalls);
            }
//            加载10点开抢活动
            List<Commodity_info> time = new Commodity_infoService().getIndexTime();
...
//          展示推荐
            List<Commodity_info> recommend = new Commodity_infoService().getRecommend();
...
//            }
            ...
            if((Enter)(request.getSession().getAttribute("e"))!=null){
                shopcarts = new ShopcartService().getShopcart(((Enter)(request.getSession().getAttribute("e"))).getEnter_id());
            }


            List<Commodity_info> yi = new Commodity_infoService().getListComm("儿童童衣");
            List<Show_info> yiImg = new ArrayList<>();
            for(Commodity_info commodity_info: yi){
                yiImg.add(new Show_InfoService().listByIdImage(commodity_info.getCommodity_info_id()));
            }

            List<Commodity_info> wan = new Commodity_infoService().getListComm("儿童玩具");
            List<Show_info> wanImg = new ArrayList<>();
            for(Commodity_info commodity_info: wan){
                wanImg.add(new Show_InfoService().listByIdImage(commodity_info.getCommodity_info_id()));
            }
            List<Commodity_info> nai = new Commodity_infoService().getListComm("儿童奶粉");
            List<Show_info> naiImg = new ArrayList<>();
            for(Commodity_info commodity_info: nai){
                naiImg.add(new Show_InfoService().listByIdImage(commodity_info.getCommodity_info_id()));
            }
            ...
            if((Enter)(request.getSession().getAttribute("e"))!=null){
                collects = new CollectService().getEnterAllById(((Enter)(request.getSession().getAttribute("e"))).getEnter_id());
                buyshows = new BuyShowDao().byShowInfo(((Enter)(request.getSession().getAttribute("e"))).getEnter_id());
            }
//            显示有多个是收藏
...
//            request.getSession().setAttribute("");
    ...
    public List<Show_info> getImage(int type){
        ...
        if(type == 1) {
            commodity_infos = new Commodity_infoService().getRecommend();
        }else {
            commodity_infos = new Commodity_infoService().getIndexTime();
        }
        ...
        for (Commodity_info commodity_info : commodity_infos){
            if(new Show_InfoDaoService().listByIdImage(commodity_info.getCommodity_info_id())!=null){
                listTime.add(new Show_InfoDaoService().listByIdImage(commodity_info.getCommodity_info_id()));
            }else {
