...
public class EnterServlet extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if("add".equals(op)){
            ...
            int ret = new EnterService().addEnter(enter);
            if(ret>0){
...
//            这个是判断手机号是否重复
            String phone = request.getParameter("phone");
            int ret = new EnterService().pattPhone(phone);
            out.print(ret);
...
//            这个是发送验证码
            ...
            int count = new EnterService().codeId(id);
            out.print(count);
            System.out.println(count+"输出验证码，进行判断");
        }else if("sel".equals(op)){
            String name=request.getParameter("name");
            String pwd=request.getParameter("pwd");
            Enter e=new EnterService().selectLogin(name, pwd);
            if(e!=null){
