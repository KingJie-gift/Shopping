...
public class Commodity_infoServletw extends HttpServlet {
...
//            查询是否是商品或者品牌进行查询
            ...
            int sum = new Commodity_infoServicew().sumCountAll(Integer.parseInt(comm),Integer.parseInt(paent_id),like,Integer.parseInt(commType),Integer.parseInt(type),Integer.parseInt(day));
            System.out.println(sum+"行数");
            pg.setSumCount(sum);
            pg.setIndexPage(Integer.parseInt(index));
//
            List<Commodity_info> commodity_infos = new Commodity_infoServicew().getByLimit(((pg.getIndexPage()-1)*pg.getRow()),pg.getRow(),Integer.parseInt(type),Integer.parseInt(comm),like,Integer.parseInt(paent_id),Integer.parseInt(commType),Integer.parseInt(day));
            ...
            for (Commodity_info commodity_info : commodity_infos){
                if(new Show_InfoDaoServicew().listByIdImage(commodity_info.getCommodity_info_id())!=null){
                    groupImage.add(new Show_InfoDaoServicew().listByIdImage(commodity_info.getCommodity_info_id()));
                }else {
...
//            显示推荐
            List<Commodity_info> dayShow = new Commodity_infoServicew().dayShow();

            List<Show_info> dayShowImage = new ArrayList<>();
            for (Commodity_info commodity_info : dayShow){
                if(new Show_InfoDaoServicew().listByIdImage(commodity_info.getCommodity_info_id())!=null){
                    dayShowImage.add(new Show_InfoDaoServicew().listByIdImage(commodity_info.getCommodity_info_id()));
                }else {
                    Show_info show_info = new Show_info();
                    show_info.setShow_info_url("img/Aptamil 爱他美卓萃幼儿配方奶粉3段1-3岁 (白金版)宝宝牛奶粉/img1.png");
                    Commodity_info commodity_info1 = new Commodity_info();
...
//            显示商品
            ...
            List<Commodity_info> shopping = new Commodity_infoServicew().getTShopping(Integer.parseInt(sid));
            String commshow = "";
            for(String s : sb){
                commshow += s;
            }
            commshow =  commshow.replaceAll(",","");
            List<Show_info> showImage = new Show_InfoServicew().getShow_infoById(Integer.parseInt(sid));
            Collect collects = null;
            if((Enter)(request.getSession().getAttribute("e"))!=null){
                collects = new CollectServicew().coll(((Enter)(request.getSession().getAttribute("e"))).getEnter_id(),Integer.parseInt(sid));
            }
            ...
            for(Commodity_info commodity_info: shopping){
                shoppingImg.add(new Show_InfoServicew().listByIdImage(commodity_info.getCommodity_info_id()));
...
//            显示适用年龄
            List<Abapt> applets = new AbaptServicew().getListAb(Integer.parseInt(sid));
